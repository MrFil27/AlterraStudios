...
if(!isset($_GET['date']) || !isset($_GET['fascia_oraria']) || !isset($_GET['user']) || !isset($_GET['class'])){
...
$stmt = $conn->prepare("SELECT classe_id FROM docenti_classi WHERE user_id = ?");
$stmt->bind_param("i", $logged_user_id);
$stmt->execute();
$result = $stmt->get_result();
$classes = [];
while($row = $result->fetch_assoc()) $classes[] = $row['classe_id'];
$stmt->close();

if(empty($classes)){
    header("Location: teacher.php?&user=" . urlencode($logged_user_id) . "&error=no-class-assigned");
    exit;
}

$placeholders = implode(',', array_fill(0, count($classes), '?'));
$types = str_repeat('i', count($classes));
$sql = "SELECT a.id, a.user_id, a.classe_id, a.teacher_id, a.descrizione, a.data,
        ...
        WHERE a.classe_id IN ($placeholders)
        ORDER BY a.data DESC, u.cognome";
...
$ammonizioni = [];
while($row = $result->fetch_assoc()) $ammonizioni[] = $row;
...
foreach($ammonizioni as $nota){
    $studentId = $nota['user_id'];
    $formattedDate = $nota['data'];

    $matrix[$formattedDate][$studentId][] = $nota;

    if(!isset($students[$studentId])) $students[$studentId] = decrypt($nota['studente_cognome']) . ' ' . decrypt($nota['studente_nome']);
}
...
<body>
    ...
    <div class="container">
        ...
        <main>
            <h1><a href="class.php?date=<?= urlencode($date)?>&fascia_oraria=<?= urlencode($fascia_oraria) ?>&user=<?=urlencode($logged_user_id)?>&class=<?=urlencode($class_id)?>">⟵ Torna indietro</a></h1>
            <br><br>

            <h1>Ammonizioni studenti</h1>
            <div class="user-details-card">
                <table>
                    <tr>
                        <th>Data / Studente</th>
                        <?php foreach($students as $sName): ?>
                            <th><?= htmlspecialchars($sName) ?></th>
                        <?php endforeach; ?>
                    </tr>

                    <?php foreach($matrix as $date => $row): ?>
                        <tr>
                            ...
                            <?php foreach($students as $sId => $sName): ?>
                                <td>
                                    <?php 
                                    if(isset($row[$sId])){
                                        foreach($row[$sId] as $nota){ 
                                            $desc = decrypt($nota['descrizione']); 
                                            echo '<span class="material-icons-sharp note-tooltip" data-desc="' . htmlspecialchars($desc, ENT_QUOTES) . '">description</span>';
                                        }
                                    }
                                    ?>
                                </td>
